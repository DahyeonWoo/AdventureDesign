        ...
        if(SensorValue[S2]==5 || SensorValue[S3]==5)
        {
            ...
            if(dest_num != 0){
                for(int i=0; i<dest_num; i++){
                    if(row_dest-3<=row_dest && row_dest<=row_destination[i]+3 && col_destination[i]-3 <= col_dest && col_dest <= col_destination[i]+3){ 
            ...
            total_reward += getReward(getState());
...
int getState()
{
    ...
    else if(SensorValue[S2]==3 && SensorValue[S3]==3)
    {
        return 3;
    }
    else if(SensorValue[S2]==5 || SensorValue[S3]==5)
    {
        return 2;
    }
    else if(SensorValue[S2]==3 || SensorValue[S3]==3)
...
task main()
{
    ...
    {
        total_reward = 0;
        total_reward += getReward(getState()); //+100
        energy();
        Home();
        if(total_reward == 0){ // 가장 잘 배움, perfect //패턴, 목적지 기억해두기
            playSound(soundBeepBeep);
            honor_destination[0] = row_destination[dest_num-1];
            honor_destination[1] = row_destination[dest_num-1];
        }
    }
}
...
void random_move(){
    r = random(9); //0~8(n-1)의 무작위 수

    if(r==0) LeftTurn();
    if(r==1) RightTurn();
    if(r==2||r==3||r==5||r==6||r==7||r==8) Forward();
}