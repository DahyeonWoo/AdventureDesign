        ...
        if(SensorValue[S2]==5 || SensorValue[S3]==5)
        {
            if(dest_num != 0){
                for(int i=0; i<dest_num; i++){
                    if(row_dest-3<=row_destination[i] && row_destination[i]<=row_dest+3 && col_dest-3<=col_destination[i] && col_destination[i]<=col_dest+3){ 
...
/*
void learn(a,b,c) //회전각도 학습, 어느정도 회전하는게 적절할지 학습한다.
{
    //출력과 정답사이의 오차E 에 따라 가중치 변경
    //출력값이 정답에 수렴 할 때까지 반복, 정답을 근사하는 함수 f(a,b,c) 구현 가능, 신경망이 커질수록 정확해짐
    //X=w1*a+w2*b+w3*c, E 생성, 가중치 변경
    while(true)
    {
        answer = 1;
        X = w1*a+w2*b+w3*c;
...
void random_move(){
    r = random(5); //0~2의 무작위 수

    if(r==0) LeftTurn();
    if(r==1) RightTurn();
    if(r==2||r==3||r==4) Forward();
}
